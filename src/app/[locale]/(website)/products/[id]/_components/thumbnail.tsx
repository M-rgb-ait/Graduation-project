...
export default function CarouselWithThumbs({ product }: { product: Product }) {
  ...
  return (
    <div className="mx-auto w-full max-w-lg space-y-4 ps-0 md:ps-10 lg:ps-0">
      {/* Main Carousel */}
      <Carousel setApi={setApi} className="w-full overflow-hidden rounded-md">
        <CarouselContent>
      ...
      <div className="flex h-28 justify-center gap-2 overflow-x-auto px-2">
        {product.images.map((src: string, index: number) => (
          <button
            ...
            className={
              "relative h-full w-1/6 overflow-hidden rounded-md border-0 brightness-75 transition-all duration-300 hover:brightness-100 active:border-2 active:border-maroon-600 active:brightness-100"
            }
          >
            <Image
              src={src}
